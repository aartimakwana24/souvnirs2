...
//         //  let SelctedFilterVArient = filteredProducts;
//         //  if (SelctedFilterVArient?.[0]?.variants?.[0]?.varients?.[0]) {
//         //   SelctedFilterVArient?.[0]?.variants?.[0]?.varients?.[0] &&
//         //     Object.keys(
//         //       SelctedFilterVArient?.[0]?.variants?.[0]?.varients?.[0]
//         //     ).forEach((key) => {
//         //       defaultSelections[key] =
//         //         SelctedFilterVArient?.[0]?.variants?.[0]?.varients?.[0][key];
//         //     });
//         //  }
  ...
  const getProducts = useCallback(async () => {
    ...
    try {
      ...
      if (data && data.collectionConditionId) {
        ...
        // Function to check if the variant has valid details
        function hasValidDetails(details) {
          return details.some(
            (detail) => detail.price > 0 && detail.quantity > 0
          );
        }
        ...
        const filteredProducts = productVariants
          .map((product) => {
            const matchingVariants = product.variants.filter((variantObj) => {
              const hasMatchingVariant = variantObj.varients.some((variant) =>
                isVariantMatching(variant, filterMap)
              );

              if (hasMatchingVariant && !hasValidDetails(variantObj.details)) {
                // swalError(
                //   "Warning",
                //   "This varient not available currently.",() => {
                //     setShowModal(false);
                //   }
                // );
              }
              return hasMatchingVariant && hasValidDetails(variantObj.details);
            });
        ...
        // Optional: Set defaultSelections if needed
        // let defaultSelections = {};
        // const selectedVariant = filteredProducts?.[0]?.variants?.[0]?.varients?.[0];
        // if (selectedVariant) {
        //   Object.keys(selectedVariant).forEach((key) => {
        //     defaultSelections[key] = selectedVariant[key];
        //   });
        // }
        // setDefaultSelections(defaultSelections);
      }
          ...
          {loading ? (
            ...
            <div className="row">
              ...
              {productVariantDetails.length > 0
                ...
                : "This Product not available currently."}
            </div>
          )}
        </div>
      </div>
